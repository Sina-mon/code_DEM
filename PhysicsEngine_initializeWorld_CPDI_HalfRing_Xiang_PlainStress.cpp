...
//			if(thisMP->d3_Position.y < d3Center.y - 0.25*d3Dimension.y)
	...
	d_TimeIncrement_Maximum = 8.0e-8;
	d_TimeEnd = 2.0*1.0*dDiameter_Outer / glm::abs(dPlatenSpeed);
	...
	{// log file name, saved in this->str_LogFile
		time_t rawtime;
		struct tm * timeinfo;
		char buffer[80];
		...
		this->str_LogFile = "./bmp/log_" + strTime + ".txt";
	}
